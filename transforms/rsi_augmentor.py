...
from eveleaf.transforms import utils
...
class Compose:
    ...
    def __call__(self, img, img_info=None, label=None):
        ...
        if label is not None:
            label = utils.read_img(label, True)
        # 数据增强
...
class Normalize:
    ...
    def __call__(self, img, img_info=None, label=None):
        ...
        img = utils.normalize(img, self.min_val, self.max_val, mean, std, self.band_num)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
...
class RandomFlip:
    ...
    def __call__(self, img, img_info=None, label=None):
        if random.random() < self.prob:
            img = utils.mode_flip(img, self.direction)
            if label is not None:
                label = utils.mode_flip(label, self.direction)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
...
class RandomRotate:
    ...
    def __call__(self, img, img_info=None, label=None):
        ...
        if random.random() < self.prob:
            img = utils.rotate_img(img, ang)
            if label is not None:
                label = utils.rotate_img(label, ang, ig_pix=self.ig_pix)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
...
class RandomEnlarge:
    ...
    def __call__(self, img, img_info=None, label=None):
        ...
        if random.random() < self.prob:
            img = utils.enlarge_img(img, x, y, h_clip, w_clip)
            if label is not None:
                label = utils.enlarge_img(label, x, y, h_clip, w_clip)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
...
class RandomNarrow:
    ...
    def __call__(self, img, img_info=None, label=None):
        ...
        if random.random() < self.prob:
            img = utils.narrow_img(img, x_rate, y_rate)
            if label is not None:
                label = utils.narrow_img(label, x_rate, y_rate, ig_pix=self.ig_pix)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
    ...
    def __init__(self, prob=0.5, alpha_range=[0.8, 1.2], beta_range=[-10, 10], band_num=7):
        ...
        if isinstance(alpha_range, list) or isinstance(alpha_range, tuple):
            if len(alpha_range) != 2:
                raise ValueError('when alpha_range is list or tuple, it should include 2 elements, but it is {}.'
                                 .format(alpha_range))
        if isinstance(beta_range, list) or isinstance(beta_range, tuple):
            if len(beta_range) != 2:
                raise ValueError('when beta_range is list or tuple, it should include 2 elements, but it is {}.'
                                 .format(beta_range))
...
class RandomStrip:
    ...
    def __call__(self, img, img_info=None, label=None):
        ...
        if random.random() < self.prob:
            ...
            img = utils.random_strip(img, strip_num, self.direction, self.band_num)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
...
class RandomFog:
    ...
    def __init__(self, prob=0.5, fog_range=[0.03, 0.28], band_num=7):
        ...
        if isinstance(fog_range, list) or isinstance(fog_range, tuple):
            if len(fog_range) != 2:
                raise ValueError('when fog_range is list or tuple, it should include 2 elements, but it is {}.'
                                 .format(fog_range))

    def __call__(self, img, img_info=None, label=None):
        if random.random() < self.prob:
            img = utils.add_fog(img, self.fog_range, self.band_num)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
...
class RandomSplicing:
    ...
    def __call__(self, img, img_info=None, label=None):
        if random.random() < self.prob:
            img = utils.random_splicing(img, self.direction, self.band_num)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
...
class RandomRemoveBand:
    ...
    def __init__(self, prob=0.1, kill_bands=None, keep_bands=None):
        self.prob = prob
        self.kill_bands = [] if kill_bands is None else list(kill_bands)
        self.keep_bands = [] if keep_bands is None else list(keep_bands)
        if prob < 0 or prob > 1:
            raise ValueError('prob should be between 0 and 1.')
        if not (isinstance(kill_bands, list)) and kill_bands is not None:
            raise ValueError('kill_bands is list or None.')
        if not (isinstance(keep_bands, list)) and keep_bands is not None:
...
class PCA:
    ...
    def __call__(self, img, img_info=None, label=None):
        ...
        out = utils.pca(pca_bands, self.out_dim).reshape((h, w, self.out_dim))
...
class NDVI:
    ...
    def __call__(self, img, img_info=None, label=None):
        img = utils.band_comput(img, self.nir_band, self.r_band)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
...
class NDWI:
    ...
    def __call__(self, img, img_info=None, label=None):
        img = utils.band_comput(img, self.g_band, self.nir_band)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
...
class NDBI:
    ...
    def __call__(self, img, img_info=None, label=None):
        img = utils.band_comput(img, self.mir_band, self.nir_band)
        if label is None:
            return img, img_info
        else:
            return img, img_info, label
