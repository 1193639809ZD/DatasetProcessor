...
if __name__ == '__main__':
    target_list = natsorted(list(Path(r'D:\datasets\Massachusetts_Dataset\building mask').glob('*.png')))
    mask_list = natsorted(list(Path(r'D:\datasets\Massachusetts_Dataset\temp').glob('*.png')))
    output_dir = Path(r'D:\datasets\Massachusetts_Dataset\new')
    # 阿里标签
    al_map = [
        (0, 0, 0),  # 黑色 未识别
        (0, 128, 0),  # 林地
